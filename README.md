...
- **History** — last 200 requests grouped by day, each with the time it was sent, its latency and status. Clicking one brings back **the response it produced**, not just the request. Pin an entry to keep it past the 200 cap; delete or copy one from the row. `Authorization`-style headers and any vault credential are redacted before anything is written
